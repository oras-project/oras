	...
	if strings.HasPrefix(Host, "localhost:") {
		When("custom host is provided", func() {
			...
			It("should attach a file to a subject via resolve flag", func() {
				...
				prepare(Reference(Host, ImageRepo, foobar.Tag), Reference(Host, repo, foobar.Tag))

				ORAS(append([]string{"attach", "--artifact-type", "test.attach", Reference(mockedHost, repo, foobar.Tag), fmt.Sprintf("%s:%s", foobar.AttachFileName, foobar.AttachFileMedia)}, unary...)...).
					WithWorkDir(tempDir).
					MatchStatus([]match.StateKey{foobar.AttachFileStateKey}, false, 1).Exec()
				// validate
				var index ocispec.Index
				bytes := ORAS("discover", "-o", "json", Reference(Host, repo, foobar.Tag)).Exec().Out.Contents()
			...
			It("should push a blob from a stdin and output the descriptor with specific media-type", func() {
				...
				ORAS(append([]string{"blob", "push", Reference(mockedHost, repo, pushDigest), "-", "--media-type", mediaType, "--descriptor", "--size", strconv.Itoa(len(pushContent))}, unary...)...).
			...
			It("should fetch blob descriptor ", func() {
				ORAS(append([]string{"blob", "fetch", Reference(mockedHost, ImageRepo, foobar.FooBlobDigest), "--descriptor"}, unary...)...).
			...
			It("should delete a blob with force flag and output descriptor", func() {
				...
				ORAS("cp", Reference(Host, ImageRepo, foobar.Digest), Reference(Host, repo, foobar.Digest)).Exec()
				// test
				toDeleteRef := Reference(mockedHost, repo, foobar.FooBlobDigest)
				ORAS(append([]string{"blob", "delete", toDeleteRef, "--force", "--descriptor"}, unary...)...).
					MatchContent(foobar.FooBlobDescriptor).
					Exec()
				deletedRef := Reference(Host, repo, foobar.FooBlobDigest)
			...
			It("should copy an image with source resolve DNS rules", func() {
				...
				dst := Reference(Host, repo, "copied")
				ORAS(append([]string{"cp", Reference(mockedHost, ImageRepo, foobar.Tag), dst, "-v"}, from...)...).
					MatchStatus(foobarStates, true, len(foobarStates)).
					Exec()
				CompareRef(Reference(Host, ImageRepo, foobar.Tag), dst)
			})
			...
			It("should copy an image with destination resolve DNS rules", func() {
				...
				repo := testRepo("cp/to")
				tag := "copied"
				ORAS(append([]string{"cp", src, Reference(mockedHost, repo, tag), "-v"}, to...)...).
					MatchStatus(foobarStates, true, len(foobarStates)).
					Exec()
				CompareRef(src, Reference(Host, repo, tag))
			})
			...
			It("should copy an image with base resolve DNS rules", func() {
				...
				ORAS(append([]string{"cp", src, Reference(mockedHost, repo, tag), "-v"}, flags...)...).
					MatchStatus(foobarStates, true, len(foobarStates)).
					Exec()
				CompareRef(src, Reference(Host, repo, tag))
			})
			...
			It("should copy an image with overridden resolve DNS rules for both source and destination registry", func() {
				...
				ORAS(append([]string{"cp", Reference(mockedHost, ImageRepo, foobar.Tag), Reference(mockedHost, repo, tag), "-v"}, flags...)...).
					MatchStatus(foobarStates, true, len(foobarStates)).
					Exec()
				CompareRef(Reference(Host, ImageRepo, foobar.Tag), Reference(Host, repo, tag))
			})

			It("should discover direct referrers of a subject", func() {
				bytes := ORAS(append([]string{"discover", Reference(mockedHost, ArtifactRepo, foobar.Tag), "-o", "json"}, unary...)...).Exec().Out.Contents()
				var index ocispec.Index
				Expect(json.Unmarshal(bytes, &index)).ShouldNot(HaveOccurred())
			...
			It("should fetch index manifest with digest", func() {
				ORAS(append([]string{"manifest", "fetch", Reference(Host, ImageRepo, multi_arch.Tag)}, unary...)...).
					MatchContent(multi_arch.Manifest).Exec()
			})

			It("should fetch scratch config", func() {
				ORAS(append([]string{"manifest", "fetch-config", Reference(Host, ImageRepo, foobar.Tag)}, unary...)...).
					MatchContent("{}").Exec()
			})
			...
			It("should do forced deletion and output descriptor", func() {
				...
				tempTag := "to-delete"
				prepare(Reference(Host, ImageRepo, foobar.Tag), Reference(Host, repo, tempTag))
				ORAS(append([]string{"manifest", "delete", Reference(mockedHost, repo, tempTag), "-f", "--descriptor"}, unary...)...).
				...
				validateTag(Reference(Host, repo, ""), tempTag, true)
			})
			...
			It("should push a manifest from stdin without media type flag", func() {
				...
				tag := "from-stdin"
				prepare(Reference(Host, ImageRepo, foobar.Digest), Reference(Host, repo, foobar.Digest))
				//test
				ORAS(append([]string{"manifest", "push", Reference(Host, repo, tag), "-"}, unary...)...).
					MatchKeyWords("Pushed", Reference(Host, repo, tag), "Digest:", digest).
					WithInput(strings.NewReader(manifest)).Exec()
				validateTag(Reference(Host, repo, ""), tag, false)
			})
			...
			It("should pull all files in an image", func() {
				...
				// test
				ORAS(append([]string{"pull", Reference(mockedHost, ImageRepo, foobar.Tag), "-v", "--config", configName, "-o", pullRoot}, unary...)...).
			...
			It("should push files without customized media types", func() {
				repo := testRepo("push")
				tempDir := CopyTestDataToTemp()
				ORAS(append([]string{"push", Reference(mockedHost, repo, foobar.Tag), "-v"}, append(foobar.FileLayerNames, unary...)...)...).
			...
			It("should list tags", func() {
				ORAS(append([]string{"repository", "show-tags", Reference(mockedHost, ImageRepo, foobar.Digest)}, unary...)...).MatchKeyWords(foobar.Tag).Exec()
			})
			...
			It("should add a tag to an existent manifest when providing tag reference", func() {
				...
				ORAS(append([]string{"tag", Reference(mockedHost, repo, foobar.Digest), newTag}, unary...)...).MatchKeyWords(newTag).Exec()
				ORAS("repo", "tags", Reference(Host, repo, "")).MatchKeyWords(newTag).Exec()
			})
		})
	}
})
