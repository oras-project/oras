	...
	When("running `manifest delete`", func() {
		...
		It("should do forced deletion and output descriptor", func() {
			...
			ORAS("manifest", "delete", Flags.Layout, toDeleteRef, "-f", "--descriptor").
				MatchContent("{\"mediaType\":\"application/vnd.oci.image.manifest.v1+json\",\"digest\":\"sha256:fd6ed2f36b5465244d5dc86cb4e7df0ab8a9d24adc57825099f522fe009a22bb\",\"size\":851,\"annotations\":{\"org.opencontainers.image.reference.name\":\"foobar\"}}").
				Exec()
			// validate
		...
		validate := func(root string, digest string, tag string) {
			...
			for _, m := range index.Manifests {
				if m.Digest.String() == digest &&
					(tag == "" || tag == m.Annotations["org.opencontainers.image.reference.name"]) {
					return
				}
			}
