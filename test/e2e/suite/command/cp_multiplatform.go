...
var _ = Describe("Multi-platform copy users:", func() {
	When("running `cp` with multiple platforms", func() {
		It("should copy multiple platforms of image to a new repository", func() {
			...
			ORAS("cp", src, dst, "--platform", "linux/amd64,linux/arm64").Exec()
		...
		It("should fail to copy multiple platforms when some platforms are not available", func() {
			...
			ORAS("cp", src, dst, "--platform", "linux/amd64,linux/nonexistent").
				ExpectFailure().
				MatchErrKeyWords("unmatched platforms: [linux/nonexistent]", "available platforms").
				Exec()
		})
		...
		It("should copy a single platform when only one platform is specified", func() {
			...
			Expect(json.Unmarshal(manifest, &image)).ShouldNot(HaveOccurred())
			Expect(image.MediaType).To(Equal(ocispec.MediaTypeImageManifest))
		})

		It("should preserve the root when every platform is selected", func() {
			src := RegistryRef(ZOTHost, ImageRepo, ma.Tag)
			dst := RegistryRef(ZOTHost, cpMultiPlatformTestRepo("all-platforms"), "copiedAll")

			sourceDigest := strings.TrimSpace(string(ORAS("resolve", src).Exec().Out.Contents()))
			ORAS("cp", src, dst, "--platform", "linux/amd64,linux/arm64,linux/arm/v7").Exec()
			destinationDigest := strings.TrimSpace(string(ORAS("resolve", dst).Exec().Out.Contents()))
			Expect(destinationDigest).To(Equal(sourceDigest))
		})
