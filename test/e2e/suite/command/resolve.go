...
var _ = Describe("ORAS beginners:", func() {
	When("running resolve command", func() {
		...
		It("should fail when repo is invalid", func() {
			ORAS("resolve", fmt.Sprintf("%s/%s", ZOTHost, InvalidRepo)).ExpectFailure().MatchErrKeyWords("Error:", fmt.Sprintf("%s/%s", ZOTHost, InvalidRepo), "invalid reference", "<registry>/<repo>[:tag|@digest]").Exec()
		})
